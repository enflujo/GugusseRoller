...
class previewWindowWidget(QGlPicamera2):
    ...
    def mousePressEvent(self, event):
        ...
        buffers, metadata = self.win.picam2.capture_buffers(["main"])
        img = self.win.picam2.helpers.make_image(buffers[0], cfg["main"])

        focus_score = self.measure_focus(img)
        self.win.out.append(f"Focus score: {focus_score:.2f}")
