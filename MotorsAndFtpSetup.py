...
saveModeSelect = OptionMenu(
    miniFrame, saveMode, "ftp", "local", command=modeChangeHandler
)
...
filePathButton = Button(
    miniFrame, text=hardware["localFilePath"], command=getTheFileDialog
)
...
def testFeeder():
    ...
    m.blindMove(2000)
    messagebox.showinfo(
        "FEEDER PLATEAU", "The feeder plateau should have turned clockwise"
    )
...
def testFilmdrive():
    ...
    messagebox.showinfo(
        ...
        "The skateboard wheel should have turned counter-clockwise",
    )
...
def testPickup():
    ...
    m.blindMove(2000)
    messagebox.showinfo(
        "PICKUP PLATEAU", "The pickup plateau should have turned clockwise"
    )
